...
Исправления:
- Ссылки принимаются как текст (не "обрабатываются" как файлы). Для админов добавляется кнопка "Відкрити посилання".
- При любых ошибках при сохранении ссылки/файла создаётся fallback-запись в БД, чтобы callback-кнопки ссылались на реальный receipt_id.
- Если скачивание файла не удалось, бот пересылает (forward) исходное сообщение админам и сохраняет запись с file_id.
- При подтверждении/отклонении админом бот редактирует то же сообщение (caption или text) и показывает всплывашку "Прийнято"/"Відхилено".
- Минимальные изменения логики, чтобы UX был предсказуемым и надёжным.
"""
...
def cmd_start(message: types.Message) -> None:
    text = (
        ...
        "Щоб додати квитанцію, просто напишіть ім'я та прізвище дитини🧒.\n"
        "Після цього оберіть тип оплати: Позанятійне або Абонемент, і надішліть фото квитанції📷.\n"
        "Також можна надіслати файл (PDF, JPG, DOCX) або посилання — бот обробить їх як квитанцію.\n\n"
        "Щоб керувати дітьми (додати/змінити/видалити), натисніть кнопку «Керувати дітьми» нижче."
    )
...
def handle_text(message: types.Message) -> None:
    ...
    # =========================
    # Обработка ссылок (новая логика, без изменения текста пользователя)
    # =========================
    if URL_RE.match(text):
        raw_link = text  # сохраняем как пришло, не меняем текст
        # кнопка для открытия: добавляем http только в URL кнопки (не меняем raw_link в БД)
        button_url = raw_link if raw_link.startswith("http://") or raw_link.startswith("https://") else ("http://" + raw_link)
        ...
        if not children:
            ...
            return
        if len(children) > 1:
            pending_links[user_id] = {"link": raw_link}
            markup = types.InlineKeyboardMarkup()
            for ch in children:
                cid, name, ptype, note, created = ch
                label = f"{name}" + (f" — {ptype}" if ptype else "")
                markup.add(types.InlineKeyboardButton(text=label, callback_data=f"choosechild_link:{cid}"))
            bot.reply_to(message, "Оберіть, за яку дитину ця квитанція (посилання):", reply_markup=markup)
            return

        # один ребенок
        child_id, name, payment_type, note, created = children[0]
        if not payment_type:
        ...
        # Попытка сохранить; при ошибке вставляем fallback-запись, чтобы получить receipt_id
        try:
            receipt_id = save_receipt(user_id, child_id, None, None, raw_link, payment_type)
        except Exception as e:
            logger.exception("Error saving link receipt (will insert fallback): %s", e)
            with db_lock:
                cur = conn.cursor()
                ts = datetime.now(timezone.utc).isoformat()
                ...
                cur.execute(
                    "INSERT INTO receipts (user_id, child_id, file_path, file_id, link, timestamp, month, payment_type, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')",
                    (user_id, child_id, None, None, raw_link, ts, month_str, payment_type)
                )
                conn.commit()
        ...
        # Кнопка открытия ссылки (url) + кнопки подтверждения/отклонения (callback с реальным id)
        link_button = types.InlineKeyboardButton(text="Відкрити посилання", url=button_url)
        markup = types.InlineKeyboardMarkup()
        ...
        bot.reply_to(message, "Посилання отримано і надіслано на перевірку. Можете додати ще квитанцію.")
        return
...
def handle_setpay(call: types.CallbackQuery) -> None:
    try:
        ...
        bot.send_message(call.from_user.id, f"Тип оплати збережено: {payment_type}. Тепер надішліть фото/файл/посилання квитанції.")
    except Exception as e:
...
def handle_manage(call: types.CallbackQuery) -> None:
    try:
        ...
        if action == "edit":
            ...
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton(text="Змінити ім'я та прізвище", callback_data=f"editname:{cid}"),
                       types.InlineKeyboardButton(text="Змінити тип оплати", callback_data=f"editpay:{cid}"),
                       types.InlineKeyboardButton(text="Додати/змінити примітку", callback_data=f"editnote:{cid}"))
            bot.answer_callback_query(call.id)
